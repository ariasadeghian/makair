...
class TestGetOrCreateUser:
    def test_creates_when_missing(self, session):
        user = transactions.get_or_create_user(session, USER_ID, business_name="کافه‌ی من")
        assert isinstance(user, User)
        assert user.id == USER_ID
        assert user.business_name == "کافه‌ی من"

    def test_returns_existing(self, session):
        first = transactions.get_or_create_user(session, USER_ID)
        second = transactions.get_or_create_user(session, USER_ID)
...
class TestAddAndList:
    def test_add_transaction_persists(self, session):
    ...
    def test_list_orders_by_occurred_at(self, session):
        transactions.get_or_create_user(session, USER_ID)
        transactions.add_transaction(
            session, USER_ID, kind=Kind.EXPENSE, amount=100,
            category="حمل و نقل", description="", occurred_at=_dt(1403, 5, 10),
        )
        transactions.add_transaction(
            session, USER_ID, kind=Kind.INCOME, amount=200,
            category="فروش کالا", description="", occurred_at=_dt(1403, 5, 2),
        )
        start, end = _dt(1403, 5, 1, 0, 0), _dt(1403, 5, 31, 23, 59)
        rows = transactions.list_transactions(session, USER_ID, start, end)
        assert [r.amount for r in rows] == [200, 100]  # صعودی بر occurred_at

    def test_list_filters_by_range(self, session):
        transactions.get_or_create_user(session, USER_ID)
        transactions.add_transaction(
            session, USER_ID, kind=Kind.EXPENSE, amount=50,
            category="قبوض", description="", occurred_at=_dt(1403, 4, 20),
        )
        transactions.add_transaction(
            session, USER_ID, kind=Kind.EXPENSE, amount=70,
    ...
    def test_list_filters_by_kind(self, session):
        transactions.get_or_create_user(session, USER_ID)
        transactions.add_transaction(
            session, USER_ID, kind=Kind.EXPENSE, amount=50,
            category="قبوض", description="", occurred_at=_dt(1403, 5, 5),
...
class TestSummary:
    def _seed(self, session):
        transactions.get_or_create_user(session, USER_ID)
        transactions.add_transaction(
            session, USER_ID, kind=Kind.INCOME, amount=1_000_000,
            category="فروش کالا", description="", occurred_at=_dt(1403, 5, 2),
        )
        transactions.add_transaction(
            session, USER_ID, kind=Kind.INCOME, amount=500_000,
            category="درآمد خدمات", description="", occurred_at=_dt(1403, 5, 3),
        )
        transactions.add_transaction(
            session, USER_ID, kind=Kind.EXPENSE, amount=300_000,
            category="قبوض", description="", occurred_at=_dt(1403, 5, 4),
        )
        transactions.add_transaction(
            session, USER_ID, kind=Kind.EXPENSE, amount=200_000,
            category="قبوض", description="", occurred_at=_dt(1403, 5, 5),
        )
        transactions.add_transaction(
            session, USER_ID, kind=Kind.EXPENSE, amount=150_000,
    ...
    def test_totals(self, session):
        self._seed(session)
        start, end = _dt(1403, 5, 1, 0, 0), _dt(1403, 5, 31, 23, 59)
        s = transactions.summary(session, USER_ID, start, end)
        assert s["income"] == 1_500_000
        assert s["expense"] == 650_000
        assert s["balance"] == 850_000
        assert s["count"] == 5

    def test_category_grouping(self, session):
        self._seed(session)
        start, end = _dt(1403, 5, 1, 0, 0), _dt(1403, 5, 31, 23, 59)
        s = transactions.summary(session, USER_ID, start, end)
...
class TestDeleteLast:
    def test_delete_last_removes_newest(self, session):
        transactions.get_or_create_user(session, USER_ID)
        transactions.add_transaction(
            session, USER_ID, kind=Kind.EXPENSE, amount=100,
            category="قبوض", description="اولی", occurred_at=_dt(1403, 5, 1),
        )
        last = transactions.add_transaction(
            session, USER_ID, kind=Kind.EXPENSE, amount=200,
            category="قبوض", description="دومی", occurred_at=_dt(1403, 5, 2),
        )
        removed = transactions.delete_last(session, USER_ID)
        assert removed is not None
        assert removed.id == last.id
        assert removed.amount == 200
        start, end = _dt(1403, 5, 1, 0, 0), _dt(1403, 5, 31, 23, 59)
        remaining = transactions.list_transactions(session, USER_ID, start, end)
        assert len(remaining) == 1
        assert remaining[0].description == "اولی"

    def test_delete_last_returns_none_when_empty(self, session):
        transactions.get_or_create_user(session, USER_ID)
        assert transactions.delete_last(session, USER_ID) is None
