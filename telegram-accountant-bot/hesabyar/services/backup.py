...
def download_spreadsheet_xlsx(settings, out_path: str) -> Optional[str]:
    """اسپردشیتِ **مرکزی** (رجیستری) را به‌صورت xlsx دانلود می‌کند (Drive API).

    از زمان معماری چند-اسپردشیتی، دفترِ هر کسب‌وکار در اسپردشیت اختصاصیِ خودش
    است و اینجا نمی‌آید؛ آن‌ها هرکدام History نسخه‌ی گوگل را دارند و کاربر هم
    با ``/backup`` خروجی کاملِ خودش را می‌گیرد. این تابع برای پشتیبانِ لایه‌ی
    حساب‌ها (کاربرها، اشتراک‌ها، پرداخت‌ها) است.
